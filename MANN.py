...
class MANNCell():
    ...
    def __call__(self, x, prev_state):
        ...
        weights = tf.Variable(tf.random_normal([controller_output.get_shape()[0],controller_output.get_shape()[2], total_parameter_num], stddev=0.35))
        ...
        #update the memory
        M_ = prev_M * tf.expand_dims(1. - tf.one_hot(prev_indices[:,:, -1], self.memory_size), dim=3)
        ...
        with tf.variable_scope('reading'):
            for i in range(self.head_num):
                read_vector = tf.reduce_sum(tf.expand_dims(w_r_list[i], dim=3) * M, axis=2)
    ...
    def read_head_addressing(self, k, prev_M):
        ...
        k_norm = tf.sqrt(tf.reduce_sum(tf.square(k), axis=2, keep_dims=True))
        M_norm = tf.sqrt(tf.reduce_sum(tf.square(prev_M), axis=3, keep_dims=True))
    ...
    def zero_state(self,batch_size,dtype):
        one_hot_weight_vector = np.zeros([batch_size,self.samples_per_batch, self.memory_size])
        one_hot_weight_vector[..., 0] = 1
        one_hot_weight_vector = tf.constant(one_hot_weight_vector, dtype=tf.float32)
        with tf.variable_scope('init', reuse=self.reuse):
            state = {
                'read_vector_list': [tf.zeros([batch_size,self.samples_per_batch, self.memory_vector_dim])
                                     for _ in range(self.head_num)],
                'w_r_list': [one_hot_weight_vector for _ in range(self.head_num)],
                'w_u': one_hot_weight_vector,
                'M': tf.constant(np.ones([batch_size,self.samples_per_batch ,self.memory_size, self.memory_vector_dim]) * 1e-6, dtype=tf.float32)
            }
            return state